...
//------------------------Parameter----------------------
`define TV_IN_gmem0 "../tv/cdatafile/c.image_accel.autotvin_gmem0.dat"
`define TV_OUT_gmem0 "../tv/rtldatafile/rtl.image_accel.autotvout_gmem0.dat"
...
parameter gmem0_C_DATA_BITWIDTH = 32'd 8;
parameter gmem0_transaction_depth = 32'd 6220800;
parameter gmem0_mem_depth = 32'd 1555200;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem0_mem_depth) begin
                $display ("/home/ronny/tec/high-level-design-for-electronics/homework/homework-3/hls-gem5-rgb2gray/hls/image_accel/image_accel/hls/sim/verilog/AESL_axi_master_gmem0.v: Write request address %d exceed AXI master gmem0 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem0_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem0_mem_depth) begin
                $display ("/home/ronny/tec/high-level-design-for-electronics/homework/homework-3/hls-gem5-rgb2gray/hls/image_accel/image_accel/hls/sim/verilog/AESL_axi_master_gmem0.v: Read request address %d exceed AXI master gmem0 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem0_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < gmem0_transaction_depth; i = i + 1) begin 
          ...
          if (i%4 == 3) begin
              case(mem_page)
                  0 : gmem0_mem_0[i/4] = mem_tmp;
                  1 : gmem0_mem_1[i/4] = mem_tmp;
                  2 : gmem0_mem_2[i/4] = mem_tmp;
                  3 : gmem0_mem_3[i/4] = mem_tmp;
