...
//------------------------Parameter----------------------
`define TV_IN_gmem1 "../tv/cdatafile/c.image_accel.autotvin_gmem1.dat"
`define TV_OUT_gmem1 "../tv/rtldatafile/rtl.image_accel.autotvout_gmem1.dat"
...
parameter gmem1_C_DATA_BITWIDTH = 32'd 8;
parameter gmem1_transaction_depth = 32'd 2073600;
parameter gmem1_mem_depth = 32'd 518400;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/ronny/tec/high-level-design-for-electronics/homework/homework-3/hls-gem5-rgb2gray/hls/image_accel/image_accel/hls/sim/verilog/AESL_axi_master_gmem1.v: Write request address %d exceed AXI master gmem1 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem1_mem_depth) begin
                $display ("/home/ronny/tec/high-level-design-for-electronics/homework/homework-3/hls-gem5-rgb2gray/hls/image_accel/image_accel/hls/sim/verilog/AESL_axi_master_gmem1.v: Read request address %d exceed AXI master gmem1 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem1_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < gmem1_transaction_depth; i = i + 1) begin 
          ...
          if (i%4 == 3) begin
              case(mem_page)
                  0 : gmem1_mem_0[i/4] = mem_tmp;
                  1 : gmem1_mem_1[i/4] = mem_tmp;
                  2 : gmem1_mem_2[i/4] = mem_tmp;
                  3 : gmem1_mem_3[i/4] = mem_tmp;
...
initial begin : write_file_proc 
  ...
  integer j; 
  integer mem_page; 
  reg [gmem1_C_DATA_BITWIDTH - 1 : 0] mem_tmp; 
  reg [gmem1_DATA_BITWIDTH - 1 : 0] tmp_data;
  reg [63:0] bin_data;
  transaction_num = 0; 
  while(1) begin 
      @(posedge clk);
      ...
      mem_page = transaction_num % mem_page_num ;
      # 0.1;
      fp = $fopen(`TV_OUT_gmem1, "ab"); 
      if(fp == 0) begin       // Failed to open file 
          $display("Failed to open file \"%s\"!", `TV_OUT_gmem1); 
          $finish; 
      end 
      bin_data = gmem1_transaction_depth;
      write_binary(fp,bin_data,64);
      for(i = 0; i < gmem1_mem_depth; i = i + 1) begin 
          case(mem_page)
              0 : tmp_data = gmem1_mem_0[i];
              1 : tmp_data = gmem1_mem_1[i];
              2 : tmp_data = gmem1_mem_2[i];
              3 : tmp_data = gmem1_mem_3[i];
